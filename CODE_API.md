...
The Code Generation API allows you to generate production-ready HTML, CSS, and JavaScript code using GPT-4o.
...
```bash
curl -X POST "https://selinia.onrender.com/api/generate-code" \
...
Selinia's code generation follows these principles:
...
```javascript
const response = await fetch('https://selinia.onrender.com/api/generate-code', {
  method: 'POST',
