...
The TTS API provides high-quality text-to-speech synthesis powered by ElevenLabs, bringing Selinia's voice to life.
...
```json
{
  "text": "Hello! I'm Selinia, your AI assistant."
}
```
...
```bash
curl -X POST "https://selinia.onrender.com/api/tts" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "Welcome to Selinia AI! How can I assist you today?"
  }' \
  --output speech.mp3
```
...
Selinia uses ElevenLabs' `eleven_multilingual_v2` model with optimized settings:
...
| Voice ID | Custom | Selinia's unique voice profile |
...
async function speakText(text) {
  try {
    const response = await fetch('https://selinia.onrender.com/api/tts', {
      method: 'POST',
...
async function chatWithVoice(message) {
  // Get text response
  const chatResponse = await fetch('https://selinia.onrender.com/api/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message,
  ...
  // Speak the response
  const ttsResponse = await fetch('https://selinia.onrender.com/api/tts', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
